	...
	const Vector &vecForward, const QAngle &vecAngles, float flDiameter )
{
	...
	// *********************
	// Wenn die Waffe(n) weg genommen werden, muss der FOV auf Default gezwungen werden, denn es könnte ja sein, dass die Waffe
	// grade im Ironsight Modus war

	DevMsg( "Ironsight FOV wird resettet\n" );
	CBasePlayer *pOwner = UTIL_GetLocalPlayer();
	CBaseEntity *pFOVOwner = pOwner->GetFOVOwner();
	if( pOwner->SetFOV( pFOVOwner, 0) ) //modify these values to adjust how fast the fov is applied
	{
		DevMsg( "Ironsight HIDEHUD wird resettet\n" );
