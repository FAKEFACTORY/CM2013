...
//-----------------------------------------------------------------------------
// FAKEFACTORY
static ConVar ff_npc_can_hear_player_without_traceline( "ff_npc_can_hear_player_without_traceline", "1", 0, "Idle NPCs are able to hear player movement sounds if they can't see them. (Reverts valve behavior)" );
// FAKEFACTORY
...
bool CAI_BaseNPC::QueryHearSound( CSound *pSound )
{
	...
	if ( pSound->IsSoundType( SOUND_PLAYER ) && GetState() == NPC_STATE_IDLE && !FVisible( pSound->GetSoundReactOrigin() ) )
	{
		...
		// FAKEFACTORY AI
		// return false;
		if (ff_npc_can_hear_player_without_traceline.GetInt()) 
		{
