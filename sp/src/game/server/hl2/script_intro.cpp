...
void CScriptIntro::InputActivate( inputdata_t &inputdata )
{
	...
	{	
		...
		Warning(" - SSAO shader forbidden during script_intro\n");
		ConVar *pointer_ae_ssao  = cvar->FindVar( "ae_ssao" );
		if ( pointer_ae_ssao  )
		{

			ConVar *pointer_ae_ssao_allowed  = cvar->FindVar( "ae_ssao_allowed" );
			if ( pointer_ae_ssao_allowed  )
			{
				// Momentanten Status von ae_ssao in pointer_ae_ssao_allowed zwischenspeichern
				pointer_ae_ssao_allowed->SetValue ( pointer_ae_ssao->GetInt() );
				// ae_ssao auf 0 setzen
				pointer_ae_ssao->SetValue ( 0 );
			}
...
void CScriptIntro::InputDeactivate( inputdata_t &inputdata )
{
	m_bActive = false;
	// FAKEFACTORY ESTRANGED
	Warning(" - SSAO shader allowed again - script_intro was deactivating\n");
	ConVar *pointer_ae_ssao  = cvar->FindVar( "ae_ssao" );
	if ( pointer_ae_ssao  )
	{
		ConVar *pointer_ae_ssao_allowed  = cvar->FindVar( "ae_ssao_allowed" );
		...
		if ( pointer_ae_ssao_allowed  )
		{
			// ae_ssao auf alten wert setzen, der von der Activate Funktion gesetzt wurde
			pointer_ae_ssao->SetValue ( pointer_ae_ssao_allowed->GetInt() );
		}
	}
