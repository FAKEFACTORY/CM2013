...
// FAKEFACTORY VAR
static ConVar ff_new_running( "ff_new_running", "1", 0, "0=Off, 1=On (Default), Enables head bobbing and prevents weapons use while running" );
// FAKEFACTORY VAR
...
void CC_ToggleNightvision( void )
{
	CBasePlayer* pPlayer = UTIL_GetCommandClient();
	if ( pPlayer == NULL )
		return;

	// Nightvision schaltbar, nur wenn der HEV Suit angelegt ist
	if  ( pPlayer->IsSuitEquipped() ) 
	{

		ConVar *pNightvision  = cvar->FindVar( "ff_ppe_nightvision" );

		if ( pNightvision && pNightvision->GetInt() == 1 )
		{
			pNightvision->SetValue( 0 );
		}
		else
		{
			pNightvision->SetValue( 1);
		}
	}
}
static ConCommand toggle_nightvision("toggle_nightvision", CC_ToggleNightvision, "Toggles nightvision" );
...
void CC_ToggleIronSights( void )
{
	...
	// Folgende Zeile auskommentiert, weil in der Console: bad server command gepostet wird.
	//engine->ServerCommand( "toggle_ironsight" ); //forward to server
}
...
void CHL2_Player::ItemPostFrame()
{
	...
	// FAKEFACTORY START
	if ( ff_new_running.GetInt() == 1 )
	{
		// Schiessen auf Leitern soll erlaubt bleiben, also diesen Teil auskommentieren
		//if ( GetMoveType() == MOVETYPE_LADDER )
		//{
		//	Weapon_Lower();
		//}

		//if is sprinting and moving
		if ( IsSprinting() && !(!fabs(GetAbsVelocity().x) && !fabs(GetAbsVelocity().y)) )
		{
			//lower weapon and add sprinting effects
			Weapon_Lower();
