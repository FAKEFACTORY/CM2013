...
// FAKEFACTORY VAR
static ConVar ff_staretime( "ff_staretime", "15", 0, "Necessary time in seconds, until Alyx reacts to staring." );
...
void CNPC_Alyx::Spawn()
{
	...
	// FAKEFACTORY Alyx more health
	// This compensates the better combines
	// m_iHealth			= 80;
	m_iHealth			= 160;
	// FAKEFACTORY Alyx more health
...
void CNPC_Alyx::PrescheduleThink( void )
{
	...
	// FAKEFACTORY
	// Regelmaesig Response-System anfeuern für Look_Stare abfragen
	if (	GetTimePlayerStaring() > ff_staretime.GetInt() && !IsMoving()      )
	{
		SpeakIfAllowed( TLK_FAKEFACTORY );
	}
	// FAKEFACTORY
}
