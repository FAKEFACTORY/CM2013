...
// FAKEFACTORY Anim
// Default aus - weil buggy
static ConVar FF_Anim_FixedTurn( "FF_Anim_FixedTurn", "0" ,0, "Slow down npc turn-animations to a realistic speed (fixing old VALVE code bug)");
// FAKEFACTORY Anim
...
bool CAI_BlendedMotor::AddTurnGesture( float flYD )
{
	...
	// FAKEFACTORY Anim START
	//return false;
	if (!FF_Anim_FixedTurn.GetBool())
	{
		return false;
	}
	...
	if (seq != ACTIVITY_NOT_AVAILABLE)
	{
		...
		if (iLayer != -1)
		{
			...
			// FAKEFACTORY ANIM 2 Version langsamer
			// float rate = random->RandomFloat( 0.5, 1.1 );
			float rate = random->RandomFloat( 0.05, 0.3 );
