...
// FAKEFACTORY ViewBob
// Camera Bob
ConVar FF_viewbob_enabled	( "FF_viewbob_enabled", "0", 0, "Oscillation Toggle", true, 0, true, 1 );
ConVar FF_viewbob_timer		( "FF_viewbob_timer", "4", 0, "Speed of Oscillation");
ConVar FF_viewbob_scale_x	( "FF_viewbob_scale_x", "0.0", 0, "Magnitude of Oscillation");
ConVar FF_viewbob_scale_y	( "FF_viewbob_scale_y", "0.02", 0, "Magnitude of Oscillation");
ConVar FF_viewbob_scale_z	( "FF_viewbob_scale_z", "0.05", 0, "Magnitude of Oscillation");
// FAKEFACTORY ViewBob
...
void CGameMovement::WalkMove( void )
{
	...
	//view bob code
	if ( FF_viewbob_enabled.GetInt() == 1 && !engine->IsPaused() )
	{
		float xoffset = sin( 2 * gpGlobals->curtime * FF_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * FF_viewbob_scale_x.GetFloat() / 400;
		float yoffset = sin( 2 * gpGlobals->curtime * FF_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * FF_viewbob_scale_y.GetFloat() / 400;
		float zoffset = sin( 2 * gpGlobals->curtime * FF_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * FF_viewbob_scale_z.GetFloat() / 400;
