...
void CBaseViewModel::CalcIronsights( Vector &pos, QAngle &ang )
{
	...
	if ( !pWeapon->m_flIronsightedTime )
		return;

	float m_flIronsighted_Time_Snapshot = pWeapon->m_flIronsightedTime;

	//get delta time for interpolation
	float delta( ( gpGlobals->curtime - m_flIronsighted_Time_Snapshot ) / 0.2f ); //modify this value to adjust how fast the interpolation is
	...
	if( exp < 0.0f ) 
	{
		//DevMsg( "FAKEFACTORY DEBUG: EXP ist KLEINER NULL, wird korrigiert \n" );
		exp = 1.0f; 
		//return;
	}
	...
	if( exp > 1.0f ) 
	{
		//DevMsg( "FAKEFACTORY DEBUG: EXP ist GROESSER EINS, wird korrigiert \n" );
		exp = 0.0f; 
		return;
	}
	...
	{
		//pWeapon->m_flIronsightedTime  = 0.0f;
		return;
	}
...
// 
//	CBaseCombatWeapon *pWeapon = m_hWeapon.Get();
//		if ( pWeapon != NULL ) //war: if( pWeapon != NULL && !pWeapon->IsIronsighted() ) 
//		{
//#if defined( CLIENT_DLL )
//		if ( !prediction->InPrediction() )
//#endif
...
//		AddViewModelBob( owner, vmorigin, vmangles );
// 
//		// Add lag (wenn nicht in Ironsight)
