...
// FAKEFACTORY VAR
static ConVar ff_ironsight_disable_weapon_lowering( "ff_ironsight_disable_weapon_lowering", "0", 0, "Disables automatic Weapon lowering while in ironsight mode." );
...
bool CBaseHLCombatWeapon::WeaponShouldBeLowered( void )
{
	...
	// Das Absenken wird unterdrückt, wenn die Waffe im Ironsight Modus ist *UND* die
	// entsprechende ConVar gesetzt (default) ist.


	if( m_bIsIronsighted )
	{
		if (ff_ironsight_disable_weapon_lowering.GetInt()) 
		{
			return false;
		}
	}
...
float CBaseHLCombatWeapon::CalcViewmodelBob( void )
{
	...
	if( m_bIsIronsighted )
	{
		g_verticalBob = (g_verticalBob * 0.2);
	}
	// *****************************
	// FAKEFACTORY IRONSIGHT
	// *****************************
