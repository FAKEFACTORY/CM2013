...
// FAKEFACTORY Tracer
static ConVar FF_NoBulletTracer( "FF_NoBulletTracer", "1" ,0, "Do not show tracer lines with bullet weapons");
// FAKEFACTORY Tracer
...
//#define TWEAK_IRONSIGHTS // Comment this out for final release.
...
static ConVar ff_ironsight_disable_crosshair( "ff_ironsight_disable_crosshair", "1", 0, "Disables the crosshair in ironsight mode." );
...
void CBaseCombatWeapon::EnableIronsights( void )
{
	...
	if( !GetIronsightFOVOffset()) // Wenn der FOV Offset false ist, heisst das: Es gibt keinen Offset, also kein Ironsight
		return;
	...
	if (ff_ironsight_disable_crosshair.GetInt()) 
	{
		// Spezialfall Crossbow: Im Weaponscript ist der FOV_Offset mit -50 definiert 
		// Der Code checkt auf diesen Wert und verhindert in diesem Fall das Ausblenden des HUD Crosshairs
		if( GetIronsightFOVOffset() == -50.0f)
		{
			//Hier könnte nun eigener Zoom-Code stehen

		}
		else 
		{
			// HUD ausblenden, wenn in Ironsight
			//DevMsg( "NO CROSSBOW IRONSIGHT\n" );
			pOwner->m_Local.m_iHideHUD |= HIDEHUD_CROSSHAIR;
		}	

	}

	if( pOwner->SetFOV( this, pOwner->GetDefaultFOV()+GetIronsightFOVOffset(), 0.3f, 0.1f ) ) //modify these values to adjust how fast the fov is applied
	{
...
void CBaseCombatWeapon::DisableIronsights( void )
{
	...
	if( pOwner->SetFOV( this, 0, 0.2f, 0.1f ) ) //modify these values to adjust how fast the fov is applied
	{
		//FF_WeaponInIronsightMode.SetValue(0);
		m_bIsIronsighted = false;
		m_flIronsightedTime = gpGlobals->curtime;
		//delay next primary attack 250 ms
		m_flNextPrimaryAttack = gpGlobals->curtime + 0.25f;
	...
	// HUD einblenden, wenn nicht in Ironsight
	pOwner->m_Local.m_iHideHUD &= ~HIDEHUD_CROSSHAIR;
