...
inline bool CClient_Precipitation::SimulateRain( CPrecipitationParticle* pParticle, float dt )
{
	...
	*/
	...
	// wir beschränken auf Wasserflächen
	UTIL_TraceLine(vOldPos, pParticle->m_Pos, MASK_WATER, this, COLLISION_GROUP_NONE, &trace);
	...
	if( trace.fraction < 1 || trace.DidHit() )
	{
		if ( RandomInt( 0, 100 ) <= r_RainSplashPercentage.GetInt() )
			...
			DispatchParticleEffect( "FF_water_splash_01_froth2", trace.endpos,trace.m_pEnt->GetAbsAngles() , NULL ); 
		// water_splash_01 since this is a water impact particle which I found in the water_impact.pcf file. To preache it properly use "!particles/water_impact.pcf"
