...
// FAKEFACTORY VAR
static ConVar ff_dynamic_muzzle( "ff_dynamic_muzzle", "1", 0, "0=Off, 1=On (Default), Enables dynamic Muzzle Flash Lighting while firing weapons. Hurts performance." );
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ( m_Attachments.Count() > 0 )
		{
			...
			if  ( ff_dynamic_muzzle.GetBool() )
			{
				...
				//GetAttachment( LookupAttachment( "muzzle" ), vAttachment, angles );
				...
				dl->radius = random->RandomInt( 64, 128 );
				dl->decay = 512.0f;
