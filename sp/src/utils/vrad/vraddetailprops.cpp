...
static void ComputeLighting( DetailObjectLump_t& prop, int iThread )
{
	...
	// FAKEFACTORY BLACK Detail fix
		if ( g_fixdetaillight )
		{
			if (totalColor[0] == 0.0f)
			{
				totalColor[0] = g_fixdetaillight_value; // 0.25f;
			}

			if (totalColor[1] == 0.0f)
			{
				totalColor[1] = g_fixdetaillight_value;
			}

			if (totalColor[2] == 0.0f)
			{
				totalColor[2] = g_fixdetaillight_value;
			}
		}
		else if ( g_detaillight_minimum )
		{

			if (totalColor[0] < g_detaillight_minimum_value)
			{
				totalColor[0] = g_detaillight_minimum_value; // 0.25f;
			}

			if (totalColor[1] < g_detaillight_minimum_value)
			{
				totalColor[1] = g_detaillight_minimum_value;
			}

			if (totalColor[2] < g_detaillight_minimum_value)
			{
				totalColor[2] = g_detaillight_minimum_value;
			}
		}
	// FAKEFACTORY
