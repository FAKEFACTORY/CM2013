...
// FAKEFACTORY
bool		g_fixdetaillight = false;
float		g_fixdetaillight_value = 0.0;
bool		g_detaillight_minimum = false;
float		g_detaillight_minimum_value = 0.0;
// FAKEFACTORY
...
int ParseCommandLine( int argc, char **argv, bool *onlydetail )
{
	*onlydetail = false;
	...
	for( i=1 ; i<argc ; i++ )
	{
		...
		// FAKEFACTORY
		else if ( !Q_stricmp( argv[i], "-fixdetaillight" ) )
		{
			g_fixdetaillight = true;
			g_fixdetaillight_value = 0.0f;
			if ( ++i < argc )
			{
				g_fixdetaillight_value = (float)atof (argv[i]);
				if (g_fixdetaillight_value > 1.0)
					g_fixdetaillight_value = 1.0 / g_fixdetaillight_value;
			}
			else
			{
				Warning("Error: expected a value after '-fixdetaillight'\n" );
				return 1;
			}
		}
		else if ( !Q_stricmp( argv[i], "-detaillight_minimum" ) )
		{
			g_detaillight_minimum = true;
			g_detaillight_minimum_value = 0.0f;
			if ( ++i < argc )
			{
				g_detaillight_minimum_value = (float)atof (argv[i]);
				if (g_detaillight_minimum_value > 1.0)
					g_detaillight_minimum_value = 1.0 / g_detaillight_minimum_value;
			}
			else
			{
				Warning("Error: expected a value after '-detaillight_minimum'\n" );
				return 1;
			}
		}
		// FAKEFACTORY
